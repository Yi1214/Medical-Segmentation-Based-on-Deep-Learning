...
def get_data_training(DRIVE_train_imgs_original,    # 训练图像路径
    ...
    train_masks = load_hdf5(DRIVE_train_groudTruth)  # masks always the same，作者在代码中的mask代表ground truth
...
def extract_random(full_imgs,full_masks, patch_h,patch_w, N_patches, inside=True):
    if (N_patches%full_imgs.shape[0] != 0):
        print("N_patches: please enter a multiple of", full_imgs.shape[0])
        exit()
    ...
    int 强制转换整型
    '''
    patch_per_img = int(N_patches/full_imgs.shape[0])  #一张图片要分成多少张小图片.N_patches equally divided in the full images
    print("patches per full image: " +str(patch_per_img))
    iter_tot = 0   #iter over the total number of patches (N_patches)
    for i in range(full_imgs.shape[0]):  #loop over the full images
        k=0
        while k <patch_per_img:
            x_center = random.randint(0+int(patch_w/2),img_w-int(patch_w/2)) #random.randint(a,b)用于生成一个指定范围内的整数
            # print "x_center " +str(x_center)
            y_center = random.randint(0+int(patch_h/2),img_h-int(patch_h/2)) #random.randint(a,b)用于生成一个指定范围内的整数
            # print "y_center " +str(y_center)
            #check whether the patch is fully contained in the FOV
            if inside==True:
                if is_patch_inside_FOV(x_center,y_center,img_w,img_h,patch_h)==False:
                    continue  #如果生成的center不在FOV内，则重新生成center
...
'''
用于测试集，测试集数据的分割需要按顺序进行，而不是像训练集一样随机分割
'''
def extract_ordered(full_imgs, patch_h, patch_w):
    ...
    N_patches_tot = (N_patches_h*N_patches_w)*full_imgs.shape[0]    #patches总数
...
img_h/patch_h可能不是整数，除不尽，所以需要补零，整数patch
根据patches的数量padding(补零) test_img
'''
    ...
    for i in range(data_imgs.shape[0]):  #loop over the full images
        for x in range(width):
            for y in range(height):
                if inside_FOV_DRIVE(i,x,y,original_imgs_border_masks)==True:  # 判断是否在FOV里
